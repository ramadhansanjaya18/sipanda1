    ...
    if (formModal) {
        formModal.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const response = await fetch(`${JS_BASE_URL}/process/proses_input_perkembangan.php`, { 
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(data)
                });
                ...
                if (result.status === 'success') {
                    ...
                    setTimeout(() => {
                     ...
                     if(modalMessageDiv) {
                        ...
                        modalMessageDiv.textContent = result.message || 'Terjadi kesalahan.';
                     }
                }
            } catch (error) {
                 if(modalMessageDiv) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const imunCloseBtn = imunModal ? imunModal.querySelector('.close-button') : null;
    ...
    if (imunFormModal) {
        imunFormModal.addEventListener('submit', async function(e) {
            ...
            try {
                // Ganti BASE_URL_PROCESS menjadi JS_BASE_URL
                const response = await fetch(`${JS_BASE_URL}/process/proses_input_imunisasi.php`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(data)
                });
                ...
                if (result.status === 'success') {
                    ...
                    setTimeout(() => {
                     ...
                     if(imunModalMessageDiv) {
                        ...
                        imunModalMessageDiv.textContent = result.message || 'Terjadi kesalahan.';
                     }
                }
            } catch (error) {
                 if(imunModalMessageDiv) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Container Tabel
    const tablePemeriksaanContainer = document.getElementById('tablePemeriksaanContainer');
    const tableImunisasiContainer = document.getElementById('tableImunisasiContainer');
    ...
    if (formGenerate) {
        formGenerate.addEventListener('submit', async function(e) {
            ...
            btnSubmit.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Memuat...';
            btnSubmit.disabled = true;

            try {
                // Fetch Data (Pastikan URL sesuai)
                const response = await fetch(`${JS_BASE_URL}/process/proses_generate_laporan.php?bulan=${bulan}&tahun=${tahun}`);
                const result = await response.json();
                ...
                if (result.status === 'success') {
                    ...
                    if(resultPeriod) resultPeriod.textContent = `Periode: ${result.periode}`;
                    if(countPemeriksaan) countPemeriksaan.textContent = result.data.pemeriksaan.length;
                    if(countImunisasi) countImunisasi.textContent = result.data.imunisasi.length;

                    // Generate Tabel 1: Pemeriksaan
                    let htmlP = '';
                    if (result.data.pemeriksaan.length > 0) {
                        htmlP += '<table class="paper-table">';
                        htmlP += '<thead><tr><th width="15%">Tanggal</th><th width="35%">Nama Balita</th><th width="10%">BB (kg)</th><th width="10%">TB (cm)</th><th width="10%">LK (cm)</th><th width="20%">Kader</th></tr></thead><tbody>';
                        result.data.pemeriksaan.forEach(row => {
                            htmlP += `<tr>
                                ...
                                <td class="text-right">${row.tinggi_badan || '-'}</td>
                                <td class="text-right">${row.lingkar_kepala || '-'}</td>
                                <td>${row.nama_kader || '-'}</td>
                            </tr>`;
                        });
                        htmlP += '</tbody></table>';
                    } else {
                        htmlP += '<p style="font-style:italic; color:#777; text-align:center; padding:10px; border:1px dashed #ccc;">Tidak ada data pemeriksaan valid pada periode ini.</p>';
                    }
                    ...
                    if (result.data.imunisasi.length > 0) {
                        ...
                        htmlI += '<thead><tr><th width="15%">Tanggal</th><th width="40%">Nama Balita</th><th width="25%">Jenis Vaksin</th><th width="20%">Kader</th></tr></thead><tbody>';
                        ...
                        htmlI += '<p style="font-style:italic; color:#777; text-align:center; padding:10px; border:1px dashed #ccc;">Tidak ada data imunisasi valid pada periode ini.</p>';
                    }
    ...
    if (btnPrint) {
        btnPrint.addEventListener('click', function() {
            ...
            printWindow.document.write('<html><head><title>Cetak Laporan - SIPANDA</title>');
            // Style khusus untuk cetak (mirip dengan CSS dashboard tapi disederhanakan)
            printWindow.document.write('<style>');
            printWindow.document.write('body { font-family: "Helvetica Neue", Helvetica, Arial, sans-serif; padding: 20px; color: #000; }');
            printWindow.document.write('.report-title { text-align: center; text-transform: uppercase; font-size: 18px; margin-bottom: 5px; }');
            printWindow.document.write('.report-period { text-align: center; font-size: 14px; margin-bottom: 20px; color: #333; }');
            printWindow.document.write('.paper-divider { height: 2px; background: #000; width: 100%; margin-bottom: 30px; }');
            
            printWindow.document.write('.stats-summary-grid { display: flex; justify-content: center; gap: 40px; margin-bottom: 40px; }');
            printWindow.document.write('.stat-box { text-align: center; border: 1px solid #ccc; padding: 15px; width: 150px; }');
            printWindow.document.write('.stat-value { display: block; font-size: 24px; font-weight: bold; margin: 10px 0; }');
            printWindow.document.write('.stat-label { font-size: 12px; text-transform: uppercase; }');

            printWindow.document.write('.sub-header-report { margin-top: 30px; margin-bottom: 10px; font-size: 14px; font-weight: bold; text-decoration: underline; }');
            
            printWindow.document.write('table { width: 100%; border-collapse: collapse; font-size: 12px; margin-bottom: 20px; }');
            printWindow.document.write('th, td { border: 1px solid #000; padding: 8px; }');
            printWindow.document.write('th { background-color: #f0f0f0; text-align: left; }');
            printWindow.document.write('.text-right { text-align: right; }');
            
            printWindow.document.write('.paper-footer-sign { margin-top: 60px; display: flex; justify-content: flex-end; }');
            printWindow.document.write('.sign-box { text-align: center; width: 200px; float: right; }'); // float for print comp
            printWindow.document.write('</style>');
    ...
    if (formModal) {
        formModal.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const response = await fetch(`${JS_BASE_URL}/process/proses_input_jadwal_ajax.php`, { 
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(data)
                });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (formPeriode) {
        ...
        formPeriode.addEventListener('submit', function(e) {
            ...
            if(reportActionsContainer) reportActionsContainer.style.display = 'block';
    ...
    if (btnGenerate) {
        btnGenerate.addEventListener('click', async function(e) {
            ...
            if(reportPreviewContainer) reportPreviewContainer.style.display = 'block';
            ...
            try {
                ...
                if (result.status === 'success') {
                    ...
                    previewArea.innerHTML = generateReportHTML(result.data, result.periode);
                } else {
                ...
                previewArea.innerHTML = '<p style="color: red;">Terjadi kesalahan koneksi.</p>';
            }
        });
    }
    ...
    function generateReportHTML(data, periode) {
        ...
        if (data.pemeriksaan.length > 0) {
            html += '<table class="data-table" style="width:100%; margin-bottom: 1.5rem;"><thead>';
            html += '<tr><th>Tanggal</th><th>Nama Balita</th><th>BB (kg)</th><th>TB (cm)</th><th>LK (cm)</th><th>Diinput Oleh</th></tr>';
            html += '</thead><tbody>';
        ...
        if (data.imunisasi.length > 0) {
            ...
            html += '<tr><th>Tanggal</th><th>Nama Balita</th><th>Jenis Vaksin</th><th>Diinput Oleh</th></tr>';
            html += '</thead><tbody>';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const updateKalender = async (bulan, tahun) => {
        ...
        try {
            ...
            const response = await fetch(`${JS_BASE_URL}/process/proses_get_kalender.php?bulan=${bulan}&tahun=${tahun}`);
            const result = await response.json();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const anakCloseBtn = anakModal ? anakModal.querySelector('.close-button') : null;
    ...
    if (anakFormModal) {
        anakFormModal.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const response = await fetch(`${JS_BASE_URL}/process/proses_input_anak_ajax.php`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(data)
                });
                ...
                if (result.status === 'success') {
                    ...
                    if (anakModalMessageDiv) {
                        ...
                        anakModalMessageDiv.textContent = result.message || 'Terjadi kesalahan.';
                    }
                }
            } catch (error) {
                if (anakModalMessageDiv) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const userCloseBtn = userModal ? userModal.querySelector('.close-button') : null;
    ...
    if (userFormModal) {
        userFormModal.addEventListener('submit', async function(e) {
            ...
            try {
                // Panggil file proses AJAX baru
                const response = await fetch(`${JS_BASE_URL}/process/proses_tambah_pengguna_ajax.php`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(data)
                });
                ...
                if (result.status === 'success') {
                    ...
                    if (userModalMessageDiv) {
                        ...
                        userModalMessageDiv.textContent = result.message || 'Terjadi kesalahan.';
                    }
                }
            } catch (error) {
                if (userModalMessageDiv) {
